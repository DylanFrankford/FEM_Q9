...
% 确认右端边缘节点存在
tolerance = 1e-10; % 定义容差以处理浮点数精度问题
right_bottom_node = find(abs(nodes(:, 1) - 0.1) < tolerance & abs(nodes(:, 2) - 0) < tolerance);
if isempty(right_bottom_node)
    error('未找到右端下边缘节点！');
else
    % 获取节点对应的自由度编号
    fx_dof1 = 2 * right_bottom_node - 1;
    
    % 使用dofXX格式施加载荷
    force_loads.(sprintf('dof%d', fx_dof1)) = -load;
end

right_top_node = find(abs(nodes(:, 1) - 0.1) < tolerance & abs(nodes(:, 2) - 0.02) < tolerance);
if isempty(right_top_node)
    error('未找到右端上边缘节点！'); % 修正错误提示信息
else
    % 获取节点对应的自由度编号
    fx_dof2 = 2 * right_top_node - 1;
    
    % 使用dofXX格式施加载荷
    force_loads.(sprintf('dof%d', fx_dof2)) = load;
end
